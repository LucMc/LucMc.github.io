::: {.content-visible when-format="typst"}

```{=typst}
#let project-card(img-path, title, description) = {
  block(
    width: 100%,
    stroke: 0.5pt + rgb("#e5e7eb"),
    inset: 12pt,
    below: 8pt,
  )[
    #grid(
      columns: (80pt, 1fr),
      gutter: 12pt,
      box(
        stroke: 0.5pt + rgb("#e5e7eb"),
        image(img-path, width: 80pt),
      ),
      [
        #text(weight: "bold", fill: rgb("#111827"), size: 10pt)[#title]\
        #v(2pt)
        #text(fill: rgb("#6b7280"), size: 9pt)[#description]
      ],
    )
  ]
}

#let project-grid-card(img-path, title, description) = {
  box(
    width: 100%,
    stroke: 0.5pt + rgb("#e5e7eb"),
    inset: 10pt,
  )[
    #image(img-path, width: 100%, height: 80pt, fit: "contain")
    #v(6pt)
    #text(weight: "bold", fill: rgb("#111827"), size: 9.5pt)[#title]\
    #v(2pt)
    #text(fill: rgb("#6b7280"), size: 8.5pt)[#description]
  ]
}

#let project-grid-card-text(title, description) = {
  box(
    width: 100%,
    stroke: 0.5pt + rgb("#e5e7eb"),
    inset: 10pt,
  )[
    #v(40pt)
    #text(weight: "bold", fill: rgb("#111827"), size: 9.5pt)[#title]\
    #v(2pt)
    #text(fill: rgb("#6b7280"), size: 8.5pt)[#description]
  ]
}
```

:::

## Research & Robotics

::: {.content-visible when-format="html"}

:::: {.project-featured}
::: {.project-featured-thumb}
![](images/projects/both_stairs.JPG)
:::
::: {.project-featured-body}
::: {.meta}
Cambridge Consultants — Research Scientist
:::
### Unitree G1 Locomotion & Stair Climbing
Reinforcement learning pipelines for Unitree G1 humanoid locomotion. Developed stair-climbing policies in MuJoCo and Isaac Sim with sim-to-real transfer experiments on physical hardware. Additionally, I combined an MPC planner for zero-shot generalisation to safely navigate obstructed scenes.
:::
::::

:::: {.project-featured}
::: {.project-featured-thumb}
![](images/projects/elf_box_picking.png)
:::
::: {.project-featured-body}
::: {.meta}
Cambridge Consultants — Research Scientist
:::
### VLA Manipulation & Box Picking
Fine-tuned Vision-Language-Action models (GR00T) for robotic manipulation tasks. Context-aware grasping and box picking with Unitree G1 humanoids. I've customised existing pipelines for supervised finetuning to Reinforcement Learning post-training. This was demonstrated at Mobile World Congress (MWC) 2026.
:::
::::

:::

::: {.content-visible when-format="typst"}

```{=typst}
#project-card(
  "images/projects/both_stairs.JPG",
  "Unitree G1 Locomotion & Stair Climbing — Cambridge Consultants",
  "Reinforcement learning pipelines for Unitree G1 humanoid locomotion. Developed stair-climbing policies in MuJoCo and Isaac Sim with sim-to-real transfer experiments on physical hardware.",
)

#project-card(
  "images/projects/elf_box_picking.png",
  "VLA Manipulation & Box Picking — Cambridge Consultants",
  "Fine-tuned Vision-Language-Action models (GR00T) for robotic manipulation tasks. Context-aware grasping and box picking with Unitree G1 humanoids.",
)
```

:::

## Agentic AI

::: {.content-visible when-format="html"}

:::: {.project-featured}
::: {.project-featured-thumb}
![](images/projects/SAMX.png)
:::
::: {.project-featured-body}
::: {.meta}
Agile Loop — Lead Research Scientist
:::
### SAM-X: Computer Use Agent
VC-funded computer-use agent. Led a team of 10, combining agentic optimisations with VLM LoRA fine-tuning. I presented our work at an agentic workshop at Google Cloud as a keynote speaker/panelist.
:::
::::

:::: {.project-featured}
::: {.project-featured-thumb}
![](images/projects/lenovo-vantage-agile-loop.jpg)
:::
::: {.project-featured-body}
::: {.meta}
Agile Loop — Research Scientist (Lenovo Client)
:::
### Hybrid Edge/Cloud Model Routing
Designed a hybrid edge/cloud model routing architecture for Lenovo, optimising inference across device and cloud endpoints. Built computer vision systems for software testing and evaluation. I also conducted reinforcement and in-context learning research using custom environments and data.
:::
::::

:::

::: {.content-visible when-format="typst"}

```{=typst}
#project-card(
  "images/projects/SAMX.png",
  "SAM-X: Computer Use Agent — Agile Loop",
  "VC-funded computer-use agent. Led a team of 10, combining reinforcement learning with VLM LoRA fine-tuning. Presented to Google Cloud.",
)

#project-card(
  "images/projects/lenovo-vantage-agile-loop.jpg",
  "Hybrid Edge/Cloud Model Routing — Agile Loop (Lenovo)",
  "Designed a hybrid edge/cloud model routing architecture for Lenovo, optimising inference across device and cloud endpoints. Built CV systems and RL environments for software testing.",
)
```

:::

## Intelligent Vehicle Design

::: {.content-visible when-format="html"}

:::: {.project-featured}
::: {.project-featured-thumb}
![](images/projects/intelligent_vehicle_design.jpg)
:::
::: {.project-featured-body}
::: {.meta}
University of Surrey — Module Lecturer
:::
### Intelligent Vehicle Design Module
Designed module content and delivered lectures for the "Intelligent Vehicle Design" module. Covered python programming for planning methods (hybrid A*) and intelligent control methods (Q-Learning).
:::
::::

:::

::: {.content-visible when-format="typst"}

```{=typst}
#project-card(
  "images/projects/intelligent_vehicle_design.jpg",
  "Intelligent Vehicle Design Module — University of Surrey",
  "Designed module content and delivered lectures for the Intelligent Vehicle Design module. Covered autonomous vehicle decision making, deep Q-Learning with Noisy Networks, and LSTM-based trajectory forecasting.",
)
```

:::

## Cybersecurity & Defence

::: {.content-visible when-format="html"}

:::: {.project-featured}
::: {.project-featured-thumb}
![](images/projects/MITRE-ATTACK-Framework-Breaking-Down-Attack-Techniques-scaled.webp)
:::
::: {.project-featured-body}
::: {.meta}
QinetiQ — Software Engineer Intern
:::
### MITRE ATT&CK Automation
Developed software to automate attacks from the MITRE ATT&CK framework for red team security assessments and penetration testing.
:::
::::

:::

::: {.content-visible when-format="typst"}

```{=typst}
#project-card(
  "images/projects/mitre-attack.png",
  "MITRE ATT&CK Automation — QinetiQ",
  "Developed software to automate attacks from the MITRE ATT&CK framework for red team security assessments and penetration testing. Built voice cryptography systems in C++ and Python.",
)
```

:::

## PhD Research

::: {.content-visible when-format="html"}

:::: {.project-featured}
::: {.project-featured-thumb}
![](images/projects/dexter-veolia.webp)
:::
::: {.project-featured-body}
::: {.meta}
University of Surrey — Veolia-Sponsored PhD
:::
### RL for the DEXTER Robot
Veolia-sponsored research developing reinforcement learning for the DEXTER industrial robot, creating complex local-remote RL environments. Developed World models for sample-efficient control, Neural Lyapunov functions for stability verification, and continual reinforcement learning in JAX.
:::
::::

:::

::: {.content-visible when-format="typst"}

```{=typst}
#project-card(
  "images/projects/dexter-veolia.png",
  "RL for the DEXTER Robot — Veolia-Sponsored PhD",
  "Veolia-sponsored research developing reinforcement learning for the DEXTER industrial robot, entirely in simulation. World models for sample-efficient control, Neural Lyapunov functions for stability verification, and continual RL at scale in JAX.",
)
```

:::

## GeoSnap

::: {.content-visible when-format="html"}

:::: {.project-featured}
::: {.project-featured-thumb}
![](images/projects/Geosnap2.png)
:::
::: {.project-featured-body}
::: {.meta}
Co-Founder & CTO
:::
### GeoSnap
A vision-based localisation and navigation platform. Built the full technical stack from concept to deployment, managing architecture decisions, development, and infrastructure. I presented and secured funding as CTO. It has been used at every University of Surrey open day since October 2024.
:::
::::

:::

::: {.content-visible when-format="typst"}

```{=typst}
#project-card(
  "images/projects/Geosnap2.png",
  "GeoSnap — Co-Founder & CTO",
  "A vision-based localisation and navigation platform. Built the full technical stack from concept to deployment, managing architecture decisions, development, and infrastructure.",
)
```

:::

## Other Projects

::: {.content-visible when-format="html"}

::: {.project-grid}

:::: {.project-card}
::: {.project-thumb}
![](images/projects/big_rc_hardware_integration.jpg)
:::
::: {.project-body}
### Autonomous RC Cars
Working directly on hardware and sensor integration.
:::
::::

:::: {.project-card}
::: {.project-thumb}
![](images/projects/rc_lane_following.png)
:::
::: {.project-body}
### RC Lane Following
Developed autonomous lane following using imitation learning.
:::
::::

:::: {.project-card}
::: {.project-thumb}
![](images/projects/openai_hackathon.png)
:::
::: {.project-body}
### ClimateCode
OpenAI Hackathon project fine-tuning LLMs for compute-optimised code generation.
:::
::::

:::: {.project-card}
::: {.project-thumb}
![](images/projects/cyclopic.jpg)
:::
::: {.project-body}
### Cyclopic
Contract robotics project architecting communications and networking systems for GPS-denied environments.
:::
::::

:::: {.project-card}
::: {.project-thumb}
![](images/projects/FUNIPEDIA_1.png)
:::
::: {.project-body}
### Funipedia
Fun Wikipedia-style article platform which injects LLM generated code to produce interactable React components, images and videos. Grokathon 2026 honourable mention.
:::
::::

:::: {.project-card}
::: {.project-thumb}
![](images/projects/image_recognition.jpg)
:::
::: {.project-body}
### Facial Recognition System
Real-time face detection and recognition pipelines using computer vision.
:::
::::

:::: {.project-card}
::: {.project-thumb}
![](images/projects/finance.JPG)
:::
::: {.project-body}
### Stock Price Prediction
BSc dissertation training an LSTM model for 14-day stock price forecasting with a web interface comparing against indicator-based strategies.
:::
::::

:::: {.project-card}
::: {.project-thumb}
![](images/projects/ctf.svg)
:::
::: {.project-body}
### CTF Challenges
Treasurer of InfoSec society attending and organising security competitions.
:::
::::

:::

:::

::: {.content-visible when-format="typst"}

```{=typst}
#grid(
  columns: (1fr, 1fr),
  gutter: 8pt,
  project-grid-card(
    "images/projects/big_rc_hardware_integration.jpg",
    "Autonomous RC Cars",
    "Embedded systems combined with reinforcement learning for autonomous navigation.",
  ),
  project-grid-card(
    "images/projects/rc_lane_following.png",
    "RC Lane Following",
    "End-to-end vision-to-steering on a taped track using convolutional neural networks.",
  ),
  project-grid-card(
    "images/projects/openai_hackathon.png",
    "ClimateCode",
    "OpenAI Hackathon project — LLM fine-tuning for compute-optimised code generation.",
  ),
  project-grid-card(
    "images/projects/cyclopic.jpg",
    "Cyclopic",
    "Contract robotics — communications and networking systems for GPS-denied environments.",
  ),
  project-grid-card(
    "images/projects/FUNIPEDIA_1.png",
    "Funipedia",
    "Fun Wikipedia-style article platform. Grokathon honourable mention.",
  ),
  project-grid-card(
    "images/projects/image_recognition.jpg",
    "Facial Recognition System",
    "Real-time face detection and recognition pipelines using computer vision.",
  ),
  project-grid-card(
    "images/projects/finance.JPG",
    "Stock Price Prediction",
    "BSc dissertation — LSTM model for 14-day stock price forecasting with a web interface comparing against indicator-based strategies.",
  ),
  project-grid-card(
    "images/projects/ctf.svg",
    "CTF Challenges",
    "Security challenges — web exploitation, reverse engineering, and cryptography.",
  ),
)
```

:::
